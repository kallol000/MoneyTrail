...
export function UserIncomePopover({
  ...
  const [initialFormdata, setInitialFormdata] = useState<
    incomeFormdataRecord[]
  >([]);
  const [popoverOpen, setPopoverOpen] = useState<boolean>(false);
  const [incomeElems, setIncomeElems] = useState<ReactNode[]>([]);
  const [changeMade, setChangeMade] = useState<boolean>(false);
  const [unsavedIncomeIds, setUnsavedIncomeIds] = useState<Set<string>>(
    new Set()
  );
  const [incomeListRefresh, setIncomeListRefresh] = useState<boolean>(false);
